...
repair_prompt_combine_topn_cot_diff_post_reflection = """
We are currently solving the following issue within our repository. Here is the issue text:
--- BEGIN ISSUE ---
{problem_statement}
--- END ISSUE ---
...
A priliminary solution was generated and feedback was given. Here is the solution and feedback:
...
We have generated a potential solution for the issue. Please review the solution step by step, and evaluate if the solution correctly solves the issue.
Please provide feedback on the solution. If the solution does not address the issue correctly, please provide a brief explanation of why it does not. If the solution does address the issue correctly, please provide a brief explanation of why it does.
"""
...
def gen_reflections(raw_output, file_instruction, problem_statement, topn_content, logger, args):
    """
    Reflect on the generated repair sample. Returns True if it needs to be regenerated, False otherwise.
    Decides if output needs to be regenerated with a prompt assessing if the output properly solves the issue.
    """

    # Make a model call with the reflection prompt
    model = make_model(
        model=args.model,
        logger=logger,
    ...
    if args.mock:
        reflection_response = {"response": "NO REFLECTION"}
    else:
        reflection_response = model.codegen(reflection_prompt, num_samples=1)[0]
    logger.info(f"Reflection response:\n{reflection_response['response']}")
    reflection_output = reflection_response["response"].lower()
    return reflection_output

def reflect(samples, file_instruction, problem_statement, topn_content, logger, args):
    """
    Perform self-reflection on the generated repair samples.
    """
    def reflect_single_sample(sample):
        raw_output = sample["response"]
        reflection = gen_reflections(raw_output, file_instruction, problem_statement, topn_content, logger, args)
        return reflection

    # Use ThreadPoolExecutor to run reflections in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_threads) as executor:
        reflected_samples = list(executor.map(reflect_single_sample, samples))

    return reflected_samples

def post_reflect(samples, message_w_reflection, reflections, model, args):
    """
    Perform post-reflection on the generated repair samples.
    """
    def generate_post_reflection(i):
        message_w_reflection_feedback = message_w_reflection.replace(
            "{{solution}}", samples[i]["response"]
        ).replace(
            "{{feedback}}", reflections[i]
        )
        post_reflected = model.codegen(message_w_reflection_feedback, num_samples=1)
        return post_reflected[0]

    # Parallelize the post-reflection LLM call
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_threads) as executor:
        post_reflected_samples = list(executor.map(generate_post_reflection, range(len(samples))))

    return post_reflected_samples
...
def process_loc(loc, args, swe_bench_data, prev_o):
    ...
    reflections = reflect(
        sample_trajs, file_instruction, problem_statement, topn_content, logger, args
    )

    message_w_reflection = repair_prompt_combine_topn_cot_diff_post_reflection.format(
        repair_relevant_file_instruction=file_instruction,
        problem_statement=problem_statement,
        content=topn_content.rstrip(),
    ).strip()

    post_reflected_samples = post_reflect(
        sample_trajs, message_w_reflection, reflections, model, args
    )

    sample_responses.extend(post_reflected_samples)
...
def post_process_raw_output(
    ...
    try:
        ...
        if edited_file in file_contents:
            ...
            print(lint_success, list(prev_errors), list(errors), differ_by_empty_lines)
